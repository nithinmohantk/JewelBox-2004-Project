...
#Region " Windows Form Designer generated code "
    ...
    Dim objAd As OleDbDataAdapter
    Dim objRows As DataRow
    Dim objCols As DataColumn
    Dim objTables As DataTable
    Dim i As Integer
    Dim sql As String
...
#End Region
    ...
    Private Sub cmdLogin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLogin.Click
        ...
        rs = conn.Execute(sql)
        If rs.EOF Then
            MsgBox("user doesn't exists")
            Exit Sub
        Else
            If decrypt_pass(rs.Fields("password").Value) = Trim(txtPassword.Text) Then
                loguser = UCase(Trim(txtUser.Text))
                logpass = Trim(txtPassword.Text)
                Dim frm As New frmMain()
                Me.Hide()
                frm.Show()
                MsgBox("Login Success Mr/Mrs. " & loguser)
            Else
